...
async function handleRegistration(from, message) {
  ...
  if (session.state === STATES.REG_AWAIT_LANDMARK) {
    ...
    try {
      ...
      delete chatStates[from]; // Registration completed
      await whatsapp.sendMessage(from, `You're all set, ${customer.name}!`);
      await sendMainMenu(from, customer.name);
    } catch (error) {
      console.error('Error registering customer:', error);
      await whatsapp.sendMessage(from, 'An error occurred during registration. Please try again by typing Hi.');
    }
  }
}
